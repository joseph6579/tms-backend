...
from organisations.api.serializers.organisations import (
    OrganisationRegistrationSerializer,
    OrganisationRedactedSerializer,
    OrganisationUpdateSerializer,
    OrganisationOrderConfigurationSerializer,
)
...
class OrganisationViewSet(ReadOnlyModelViewSet):
    ...
    def get_serializer_class(self):
        serializers = {'retrieve': OrganisationRedactedSerializer}
    ...
    @action(
        detail=False,
        methods=["put"],
        url_path="(?P<config_id>[0-9a-f-]{36})/update-order-status-configuration",
        serializer_class=OrganisationOrderConfigurationSerializer,
    )
    def update_status_config(self, request, config_id=None, *args, **kwargs):
        ...
        updated_config = OrganisationConfigService.update_status_config(config=configuration, config_data=data)
