...
class DriverReadOnlyViewset(ReadOnlyModelViewSet):
    ...
    def register(self, request, *args, **kwargs):
        ...
        """
        1. Get or create driver/user account using the email and phone_number
        ...
        driver_group = data["driver_group"]
        # TODO: Send registration Confirmation - with a code that expires

        driver = get_driver_by_email(email=email) or get_driver_by_phone(phone_number=phone_number)
        if driver is None:
            # create new driver
            driver = Driver.objects.create(
                first_name=first_name, last_name=last_name, email=email, phone_number=phone_number
            )
            # TODO: Generate code and send for verification
            code = generate_code()
        ...
        # TODO: Create as offline until the location starts streaming??
    ...
    def update_location(self, request, pk=None):
        ...
        success = self.location_service.update_driver_location(str(driver.id), lat, lon)
