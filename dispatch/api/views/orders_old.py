...
from dispatch.api.serializers.orders_old import OrderSerializer, OrderReviewSerializer
...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def review(self, request, pk=None):
        ...
        serializer = OrderReviewSerializer(data=request.data, context={'request': request})
    ...
    def broadcast(self, request):
        ...
        orders = Order.objects.filter(
            id__in=order_ids, organization=request.user.organisation, status='pending', trip__isnull=True
        )
        ...
        if not batches:
            return Response({'detail': 'Could not create batches from orders'}, status=status.HTTP_400_BAD_REQUEST)

        # Broadcast each batch
        results = []
        for batch in batches:
            success = broadcast_service.broadcast_batch(batch)
            results.append({'orders': [str(order.id) for order in batch['orders']], 'broadcast_success': success})
    ...
    def accept_batch(self, request):
        ...
        if not batch_id or not driver_id:
            return Response({'detail': 'Batch ID and Driver ID are required'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if success:
            ...
            return Response({'detail': 'Failed to accept batch'}, status=status.HTTP_400_BAD_REQUEST)
